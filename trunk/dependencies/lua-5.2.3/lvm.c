...
int luaV_tostring (lua_State *L, StkId obj) {
  if (ttisnumber(obj)) {
    ...
    char s[LUAI_MAXVECTOR22STR];
    int l = lua_vector22str(s, v2value(obj));
    setsvalue2s(L, obj, luaS_newlstr(L, s, l));
    return 1;
  } else if (ttisvector3(obj)) {
    char s[LUAI_MAXVECTOR32STR];
    int l = lua_vector32str(s, v3value(obj));
    setsvalue2s(L, obj, luaS_newlstr(L, s, l));
    return 1;
  } else if (ttisvector4(obj)) {
    char s[LUAI_MAXVECTOR42STR];
    int l = lua_vector42str(s, v4value(obj));
    setsvalue2s(L, obj, luaS_newlstr(L, s, l));
    return 1;
  } else if (ttisquat(obj)) {
    char s[LUAI_MAXQUAT2STR];
    int l = lua_quat2str(s, qvalue(obj));
    setsvalue2s(L, obj, luaS_newlstr(L, s, l));
    return 1;
  } else {
    return 0;
...
void luaV_objlen (lua_State *L, StkId ra, const TValue *rb) {
  ...
  switch (ttypenv(rb)) {
    ...
    case LUA_TVECTOR2: {
      ...
      setnvalue(ra, sqrtf(f4.x*f4.x + f4.y*f4.y));
      return;
    }
    case LUA_TVECTOR3: {
      lua_Float4 f4 = v3value(rb);
...
void luaV_arith (lua_State *L, StkId ra, const TValue *rb,
  ...
  else if (ttisvector2(rb) && ttisvector2(rc)) {
 ...
 newframe:  /* reentry point when frame changes (call/return) */
  ...
  for (;;) {
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      case OP_DIV: { //fuck you define-o-phils
        TValue *rb = RKB(i);
        TValue *rc = RKC(i);
        if (ttisnumber(rb) && ttisnumber(rc)) {
          lua_Number nb = nvalue(rb), nc = nvalue(rc);
          if (nc==0.0) {
            Protect(luaG_runerror(L, "division by zero"));
          }
          nb = luai_numdiv(NULL, nb,nc);
          setnvalue(ra, nb);
        }
        else
